...
// Стартовая мелодия: бодрый мотив (ноты C5..C6, длительности мс)
void startupMelody() {
  // C5 E5 G5 C6 | G5 E5 | A5 F5 | C5 G5 C5 - (арпеджио + финал)
  const uint16_t notes[] = {
    523, 659, 784, 1047,
    784, 659,
    880, 698,
    523, 784, 1047, 880, 784, 659, 587, 523
  };
  const uint16_t durs[] = {
    160, 160, 160, 320,
    200, 200,
    200, 200,
    160, 160, 240, 160, 160, 160, 160, 480
  };
  for (uint8_t i = 0; i < 15; i++) { beep(notes[i], durs[i]); delay(30); }
}
...
void setup() {
  ...
  arduboy.display();
  arduboy.bootLogo();
  startupMelody();          // стартовая мелодия при загрузке
...
void loop() {
  ...
  switch (screen) {
    case SPLASH_INTRO: {
      ...
      arduboy.clear();
      if (show) drawLogo(LOGO_SPLASH_Y);
      break;
    }
    case SPLASH: {
    ...
    case PAINT: {
      ...
      if (!ab) {
        ...
        if (arduboy.justReleased(B_BUTTON) && !bHandled) {
          if (isCanvasEmpty()) { goScreen(MENU); break; }
          if (histHead > 0) {
